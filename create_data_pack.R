...
# organisation_code <- 'RH8'

# University Hospitals Plymouth
# ─────────────────────────────
# provider_code_selection <- 'RK9'
# organisation_name <- 'University Hospitals Plymouth (UHP)'
# organisation_code <- 'RK9'

# Torbay and South Devon
# ──────────────────────
# provider_code_selection <- 'RA9'
# organisation_name <- 'Torbay and South Devon (TSD)'
# organisation_code <- 'RA9'

# Devon ICB
# ─────────
provider_code_selection <- c('RH8', 'RBZ', 'RA9', 'RK9') 
organisation_name <- 'Devon ICB'
organisation_code <- 'QJK'
...
df_nonadm_wl_shape <- df_detail_long %>% 
  ...
  group_by(Treatment_Function_Code, RTT_Part, Weeks) %>%
...
df_adm_wl_shape <- df_detail_long %>% 
  dplyr::filter(Provider_Code %in% provider_code_selection &
                  RTT_Part== 'PART_2A') %>%
  group_by(Treatment_Function_Code, Weeks) %>%
  summarise(Volume = sum(Volume, na.rm = TRUE), .groups = 'keep') %>%
  ungroup()
...
# Loop through each treatment function code in the lookup
for(selected_tfc in df_tfc_lu$Treatment_Function_Code){
  output_dir <- paste0('output/', organisation_code, '/', selected_tfc)
  ...
  ftbl_nonadm_starts_wda <- flextable(data = df_table_data) %>%
    theme_booktabs() %>%
    add_header_row(top = TRUE, values = 'Statistics for Non-Admitted Clock Starts Adj. Working Days', colwidth = 3) %>%
    flextable::align(i = 1, j = NULL, align = 'center', part = 'header')  %>%
    set_table_properties(layout = "autofit", width = .9) %>%
    colformat_double(j = 2:3, digits = 1) %>%
    colformat_double(i = 8, j = 2:3, digits = 0) %>%
  ...
  ftbl_nonadm_stops_wda <- flextable(data = df_table_data) %>%
    theme_booktabs() %>%
    add_header_row(top = TRUE, values = 'Statistics for Non-Admitted Clock Stops Adj. Working Days', colwidth = 3) %>%
    flextable::align(i = 1, j = NULL, align = 'center', part = 'header')  %>%
    set_table_properties(layout = "autofit", width = .9) %>%
    colformat_double(j = 2:3, digits = 1) %>%
    colformat_double(i = 8, j = 2:3, digits = 0) %>%
  ...
  ftbl_adm_stops_wda <- flextable(data = df_table_data) %>%
    theme_booktabs() %>%
    add_header_row(top = TRUE, values = 'Statistics for Admitted Clock Stops Adj. Working Days', colwidth = 3) %>%
    flextable::align(i = 1, j = NULL, align = 'center', part = 'header')  %>%
    set_table_properties(layout = "autofit", width = .9) %>%
    colformat_double(j = 2:3, digits = 1) %>%
    colformat_double(i = 8, j = 2:3, digits = 0) %>%
  ...
  ftbl_nonadm_proxy_starts_wda <- flextable(data = df_table_data) %>%
    theme_booktabs() %>%
    add_header_row(top = TRUE, values = 'Statistics for Non-Admitted Proxy Clock Starts Adj. Working Days', colwidth = 3) %>%
    flextable::align(i = 1, j = NULL, align = 'center', part = 'header')  %>%
    set_table_properties(layout = "autofit", width = .9) %>%
    colformat_double(j = 2:3, digits = 1) %>%
    colformat_double(i = 8, j = 2:3, digits = 0) %>%
  ...
  ftbl_nonadm_proxy_stops_wda <- flextable(data = df_table_data) %>%
    theme_booktabs() %>%
    add_header_row(top = TRUE, values = 'Statistics for Non-Admitted Proxy Clock Stops Adj. Working Days', colwidth = 3) %>%
    flextable::align(i = 1, j = NULL, align = 'center', part = 'header')  %>%
    set_table_properties(layout = "autofit", width = .9) %>%
    colformat_double(j = 2:3, digits = 1) %>%
    colformat_double(i = 8, j = 2:3, digits = 0) %>%
  ...
  ftbl_adm_proxy_stops_wda <- flextable(data = df_table_data) %>%
    theme_booktabs() %>%
    add_header_row(top = TRUE, values = 'Statistics for Admitted Proxy Clock Stops Adj. Working Days', colwidth = 3) %>%
    flextable::align(i = 1, j = NULL, align = 'center', part = 'header')  %>%
    set_table_properties(layout = "autofit", width = .9) %>%
    colformat_double(j = 2:3, digits = 1) %>%
    colformat_double(i = 8, j = 2:3, digits = 0) %>%
...
for(selected_tfc in df_tfc_lu$Treatment_Function_Code){
  output_dir <- paste0('output/', organisation_code)
  
  rmarkdown::render(input = 'create_data_pack.Rmd', 
                    output_format = 'pdf_document', 
                    output_file = paste0('c:/temp/', organisation_code, '_', selected_tfc, '_RTT_Data_Pack.pdf'), 
                    params = list(set_title = paste0('Referral to Treatment Data Pack for ', organisation_name),
                                  set_subtitle = paste0(selected_tfc, ' - ', df_tfc_lu$Treatment_Function_Name[df_tfc_lu$Treatment_Function_Code==selected_tfc]),
                                  set_author = 'Richard Blackwell',
                                  elements_dir = paste0(output_dir, '/', selected_tfc, '/')))
}
